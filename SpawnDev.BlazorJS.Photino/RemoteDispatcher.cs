...
using MessagePackList = System.Collections.Generic.List<System.Text.Json.JsonElement>;
...
namespace SpawnDev.BlazorJS.Photino
{
    ...
    public abstract class RemoteDispatcher : AsyncCallDispatcherSlim, IDisposable
    {
        ...
        /// <summary>
        /// If true, the RemoteCallable attribute must be present on the method or the containing class
        /// </summary>
        protected bool RequireRemoteCallableAttribute { get; set; } = true;
        /// <summary>
        /// Returns true if this instance has been disposed
        /// </summary>
        public bool IsDisposed { get; protected set; }
        public JsonSerializerOptions JsonSerializerOptions { get; } = new JsonSerializerOptions { };
        public RemoteDispatcher(IServiceProvider serviceProvider, JsonSerializerOptions? jsonSerializerOptions = null, bool createNewScope = true)
        {
        ...
        /// <returns></returns>
        protected virtual async Task<string?> CanCallCheck(MethodInfo methodInfo, RemoteCallableAttribute? remoteCallableAttr, object? instance)
        {
            if (!AllowNonServiceStaticMethods && instance == null && methodInfo.IsStatic)
            {
                return "HandleCallError: Access denied to static method on non-service";
            }
            ...
            if (RequireRemoteCallableAttribute && remoteCallableAttr == null)
            {
                return "HandleCallError: Access denied not RemoteCallable";
            }
            return null;
        }
        /// <summary>
        ...
        /// <summary>
        /// Override this method to customize callable targets
        /// </summary>
        /// <param name="serviceType"></param>
        /// <returns></returns>
        protected virtual async Task<object?> GetServiceAsync(Type serviceType)
        {
            try
            {
                return await ScopedServiceProvider.GetServiceAsync(serviceType);
            }
            catch (Exception ex)
            {
                var nmt = true;
        ...
        async Task HandleCall(MessagePackList msg, bool resultRequested)
        {
            ...
            try
            {
                ...
                if (targetType == null)
                {
                    retError = $"HandleCallError: Target not found: {serviceTypeName}";
                    goto SendResponse;
                }
                // locate info about the type being called
                instance = await GetServiceAsync(targetType);
            ...
            var deniedError = await CanCallCheck(methodInfo, remoteCallableAttr, instance);
